...
def init():
    ...
    tiles = decompose_img(img, len(img), tile_size, shift) #lista kafelkow
    n_tiles = len(tiles)
    img_size = len(img)
    

    height = tile_size  # one column of image
    width = tile_size   # one row of image
    sliced = []  # new list for 8x8 sliced image
    print("The tile heigh is " + str(height) + ", and image width is " + str(width) + " pixels")
    # dividing 8x8 parts
    for t,tile in enumerate(tiles):
        currY = 0  # current Y index
        print(t,"tile cutting")
        for i in range(block, height + 1, block):
            currX = 0  # current X index
            for j in range(block, width + 1, block):
                sliced.append(tile[currY:i, currX:j] - np.ones((block, block)) * 128)  # Extracting 128 from all pixels
                currX = j
            currY = i

    print("Size of the sliced image: ", len(sliced))
    print("Each elemend of sliced list contains a ", sliced[0].shape, " element.")
